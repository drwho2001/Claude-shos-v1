...
// -----------------------
// Kane's real ask: the ability to add/rename/reorder simple option
...
// Kane asked directly: stored data lives in the browser's own storage,
...
// touches what's already stored on Kane's own device. The merge below
// (`{ ...SEED_LISTS, ...stored }`) means: a list Kane has never
// touched still picks up any future seed update automatically (e.g. if
// a future session adds a new editable category); a list Kane HAS
...
// orphan Kane's own stored edits with no warning.
//
...
// Kane's own read was that this category is unlikely to need new
...
const SEED_LISTS = {
  ...
  sampleType: ["Blood", "Urine", "Throat swab", "Rectal swab"],
  vaccine: ["Hepatitis A", "Hepatitis B", "HPV", "Mpox", "Gonorrhoea", "Other"],
...
export const OPTION_LIST_ICONS = {
  ...
  reasonForVisit: { icon: "ClipboardList", color: "#4A80F0" },
  followUpType: { icon: "CalendarClock", color: "#4A80F0" },
  sampleType: { icon: "TestTube", color: "#4A80F0" },
  vaccine: { icon: "Syringe", color: "#4A80F0" },
  vaccinationReason: { icon: "CalendarCheck", color: "#4A80F0" },
  injectionSite: { icon: "MapPin", color: "#4A80F0" },
  episodeTriggerReason: { icon: "PlayCircle", color: "#4A80F0" },
