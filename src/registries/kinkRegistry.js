...
// instead of leaving it to grow purely from what Kane happens to type.
// Two real sources, not invented:
// 1. Kane's own actual Notion Kink Registry (queried live this
...
const SEED_NAMES = [
  // From Kane's own real Notion Kink Registry, deduplicated from role-
  ...
  // Kane's own example of a pairing that needs two genuinely
  ...
  // ADDED — real ask, Kane's own explicit example of the Top/bottom
...
// when nothing exists in storage yet). Kane's own registry already has
...
// truly missing, so nothing already in Kane's registry gets duplicated
...
const KINK_SYNONYMS = {
  ...
  "wam": "Sploshing",
  // ADDED — real ask: Kane's own personal shorthand, "so you're
  ...
  // already covered above before Kane's note, confirming it was
  // already right.
  "ff": "Fisting",
};
...
// ADDED — real ask: Kane's own example (breath play = umbrella term,
...
// be guessing at what Kane actually meant, not just tidying up
...
//
// Kane's own instruction extended past just Breath play/Choking:
...
// Kane already chose deliberately) and found four more real umbrella/
...
// ADDED — real ask: Kane's own original Notion data already knew this
...
// everything, which lost real information Kane's own original design
...
//
// Classified from two real sources: Kane's own original Notion data
...
// UI. Milking keeps Kane's own original "dynamic" (Dom/sub) choice;
...
//
// Not exhaustive by design, matching Kane's own explicit scope call —
...
export const KINK_ROLE_STYLE = {
  // Anatomical — Top/bottom/Vers (from Kane's own Notion "(top)"/"(bottom)" data)
  ...
  "Felching": "anatomical", // Kane's own explicit example
  "Breeding": "anatomical",
  "Gagging": "anatomical",

  // Dynamic — Dom/sub/Switch (from Kane's own Notion "(Dom)"/"(sub)" data)
  "CBT": "dynamic",
  "Chastity": "dynamic",
  "Choking": "dynamic",
  "CNC": "dynamic",
